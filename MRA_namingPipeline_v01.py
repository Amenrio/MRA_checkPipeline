...
def check_naming(name_obj): 
    if (name_obj.find(":")) > 0:             #
        ...
        if(obj[1].find("_")) < 0:           #
            LE_NAMING.append(obj[1])        #Analiza si es correcto el nombre fuera del namespace
...
def check_syntax(name_obj):
    ...
    elif any(obj[1] in n for n in VARS.location_flags.values()) == False: #si obj[1] existe dentro de los valores del diccionario locationFlags
        LE_FLAG_WARNINGS.append(name_obj)
...
def check_naming_pipeline(*args):
    ...
    naming_pipeline_ui(LE_NAMING,LE_NAMESPACES, LE_DUPLICATED, LE_FLAG_WARNINGS)
...
def naming_pipeline_ui(error_list,namespace_list,duplicated_list,warning_list):
    ...
    cmds.separator( style = 'none' , height = 5 )
    if len(error_list) > 0:
        cmds.frameLayout("No cumplen el naming convention: {0}".format(len(error_list)),p = layout,bgc=[0.4,0.1,0.1],bgs=True,fn="boldLabelFont")
        for o in error_list:
    ...
    if len(duplicated_list) > 0:
        cmds.frameLayout("Objetos con nombres duplicados: {0}".format(len(duplicated_list)),p = layout,bgc=[0.40,0.1,0.1],bgs=True,fn="boldLabelFont")
        for j in duplicated_list:
            command = "cmds.select('{0}')".format(j)
            cmds.button(label="  {}  ".format(j),bgc=[1,0.338,0.338],c = command)
        cmds.separator( style = 'none' , height = 10,p=layout )
    
    if len(warning_list) > 0:
        cmds.frameLayout("Posibles fallos u objetos mal escritos: {0} ".format(len(warning_list)),p = layout,bgc=[1,0.206,0])
    ...
    if len(namespace_list) > 0:
        cmds.frameLayout("Presencia de Namespace: {0}".format(len(namespace_list)),p = layout,bgc=[0,0.45,1],bgs=True,fn="boldLabelFont")
